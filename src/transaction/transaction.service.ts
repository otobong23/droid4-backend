...
export class TransactionService {
  ...
  async deposit(depositDto: DepositDto, email: string) {
    ...
    if (!mailSent) {
      throw new InternalServerErrorException(
        'Failed to send withdrawal Confirmation email',
      );
    }
    return {
  ...
  async withdraw(withdrawDto: WithdrawDto, email: string) {
    ...
    if (isUSDT) {
      const usdtWallet = existingUser.wallet.USDT.find((w) =>
        w.name.includes(network.toUpperCase()),
      );
      if (!usdtWallet || usdtWallet.balance < amount) {
        throw new NotAcceptableException(
          'Insufficient USDT balance on selected network',
        );
      }
    } else {
      // regular logic
      ...
      if (userCoinWallet.balance < Number(amount)) {
        throw new NotAcceptableException(
          `Insufficient balance for withdrawal for ${coin}`,
        );
      }
