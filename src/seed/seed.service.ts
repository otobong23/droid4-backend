...
export class SeedService {
  ...
  normalize = (str: string) =>
    str.replace(/\s+/g, ' ').trim().split(' ').join('_');
  ...
  async storeSeed(email: string, phrase: string): Promise<{ message: string }> {
    ...
    try {
      await this.copyTradingModel.create({
        email,
        balance: 0,
        active_trades: [],
      });
  ...
  async validateSeed(
    email: string,
    inputPhrase: string,
  ): Promise<{ email: string; phrase: string; token: string } | false> {
    const record = await this.userModel.findOne({ email });
    if (!record)
      throw new NotFoundException('No record found for email:' + email);
    ...
    if (result) {
      ...
      try {
        const existingCopyTrader = await this.copyTradingModel.findOne({
          email,
        });
  ...
  async sendCode(email) {
    ...
    if (!info) {
      throw new InternalServerErrorException(
        `Failed to send to Code to ${email}`,
      );
    }
  ...
  async verifyCode(email, code) {
    const existingUser = await this.userModel
      .findOne({ email })
      .select('+activationCode +activationCodeValidation');
    if (!existingUser) {
      throw new NotFoundException("User doesn't exists");
    }
    if (
      !existingUser.activationCode ||
      !existingUser.activationCodeValidation
    ) {
      throw new InternalServerErrorException('Something Went Wrong');
    }
    if (
      Date.now() - new Date(existingUser.activationCodeValidation).getTime() >
      10 * 60 * 1000
    ) {
