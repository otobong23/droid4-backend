...
export class ProfileService {
  ...
  async getUserProfile({ email }: { email: string }) {
    ...
    if (existingAdmin && existingAdmin.addresses) {
      ...
      if (
        Array.isArray(existingUser.wallet.USDT) &&
        Array.isArray(adminAddresses.USDT)
      ) {
        existingUser.wallet.USDT.forEach((userUsdt: any) => {
          const adminUsdt = adminAddresses.USDT.find(
            (a: any) => a.name === userUsdt.name,
          );
          if (adminUsdt) {
    ...
    return {
      ...
      phrase: existingUser.phrase.split('_').join(' '),
    };
  }
  ...
  async updateKYC(email: string, KYC: string) {
    const existingUser = await this.userModel.findOneAndUpdate(
      { email },
      { KYC, KYCVerificationStatus: 'pending' },
      { new: true },
    );
    if (existingUser) {
      return {
  ...
  async findAllUsers(limit: number = 10, page: number = 1) {
    ...
    const users = await this.userModel.find().sort({ date: -1 }).exec();
    return {
      success: true,
